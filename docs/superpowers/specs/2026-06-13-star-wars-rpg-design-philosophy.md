...
| **T3** | No daily resources vs. high-crunch class depth. | **Cooldowns + action-economy are the resource.** Depth comes from *when/how* actions are spent and which cooldowns are live. Consumables (money-gated) and encounter-scoped pools fill the expendable role. *Per-day/per-rest is not forbidden but is **reserved sparingly for high-level abilities** — there is no baseline adventuring-day clock.* |
