...
> **Implementation note (2026-06-27):** the planned 3-file split was **consolidated into a single printed chapter, `docs/player-core/02-species.md`** (species framing + Size + Droid construct rules + all species with heritages + species feats + the 40 backgrounds). The separate `species.md` and `species-feats.md` working drafts were superseded and **removed**; `02-species.md` is the canonical source. Species feats follow the [feat-entry format](2026-06-26-feat-entry-format.md); backgrounds follow the Paizo-prose format above.
